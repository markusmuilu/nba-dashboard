...
def _get_season_type(d: pd.Timestamp) -> str:
    start = PLAYOFF_STARTS.get(d.year)
    if start and d >= start:
        return "Playoffs"
    prev = PLAYOFF_STARTS.get(d.year - 1)
    if prev and d >= prev:
        return "Playoffs"
...
def kpi(label: str, value: str, color: str = "", delta: str = "", delta_dir: str = "neu") -> None:
...
with tab1:
    ...
    if total:
        ...
        NBA_EVENTS = [
            ...
            ("2026-02-13", "All-Star break",   "#a78bfa"),
            ("2026-04-13", "Reg. season ends", "#94a3b8"),
            ("2026-04-14", "Playoffs start",   "#34d399"),
        ]
        for ev_date_str, ev_label, ev_colour in NBA_EVENTS:
            ev_ts = pd.Timestamp(ev_date_str).date()
            if daily["ds"].min() <= ev_ts <= daily["ds"].max():
    ...
    st.dataframe(hist_disp.iloc[(page-1)*page_size:(page)*page_size], use_container_width=True, hide_index=True)
...
with tab2:
    if hist.empty:
        ...
        with col_cal:
            ...
            st.caption(
                "Bars show actual accuracy for each model confidence range. "
                "The dashed line shows perfect calibration — if you're right 65 % of the time "
                "when confidence is 61-70 %, you're perfectly calibrated. "
                "Green = under-confident (performs better than claimed). "
                "Red = over-confident (performs worse than claimed)."
            )
            bins   = [50,60,70,80,90,101]
            labels = ["50-60","61-70","71-80","81-90","90+"]
            hist["conf_bucket"] = pd.cut(hist["confidence"], bins=bins, labels=labels, right=False)
            cal = (
            ...
            cal["midpoint"] = [55, 65, 75, 85, 95]
            cal["colour"]   = cal.apply(
                lambda r: "#34d399" if r["acc_pct"] >= r["midpoint"] else "#f87171", axis=1
            )
            ...
            fig_cal.add_trace(go.Scatter(
                x=cal["conf_bucket"].astype(str), y=cal["midpoint"],
                ...
                hovertemplate="%{x} midpoint: %{y}%<extra></extra>",
            ))
        ...
        odds_sub = hist.dropna(subset=["home_odds","away_odds"])
        fav_baseline = None
        if len(odds_sub):
            fav_won = (
                ((odds_sub["home_odds"] < odds_sub["away_odds"]) &  odds_sub["winner"]) |
                ((odds_sub["away_odds"] < odds_sub["home_odds"]) & ~odds_sub["winner"])
            )
            fav_baseline = fav_won.mean() * 100
        ...
        if fav_baseline is not None:
            base_labels.append("Always favourite")
            base_values.append(fav_baseline)
            base_colours.append("#34d399")
        ...
        if fav_baseline is not None:
            gap = overall_acc - fav_baseline
            color_str = "#34d399" if gap >= 0 else "#f87171"
            arrow = "▲" if gap >= 0 else "▼"
            st.markdown(f'<div style="font-size:0.78rem;color:{color_str};text-align:center">'
                        f'{arrow} {abs(gap):.1f}pp vs always-favourite baseline</div>', unsafe_allow_html=True)
...
with tab3:
    if hist.empty:
        ...
        t_wins  = int((team_hist["team"]==team_pick).sum() * team_hist[team_hist["team"]==team_pick]["winner"].mean()
                   + (team_hist["opponent"]==team_pick).sum() * (1 - team_hist[team_hist["opponent"]==team_pick]["winner"].mean())
                   if t_games else 0)
        ...
        st.dataframe(thd.sort_values("Date", ascending=False), use_container_width=True, hide_index=True)
...
with tab4:
    if hist.empty:
        ...
        if odds_hist.empty:
            ...
            bins, bucket_labels = [50,60,70,80,90,101], ["50-60","61-70","71-80","81-90","90+"]
            odds_hist["conf_bucket"] = pd.cut(odds_hist["confidence"], bins=bins, labels=bucket_labels, right=False)
            conf_upset = (
...
with tab5:
    ...
    if odds_df.empty:
        ...
        daily_pnl = (
            ...
            .agg(pnl_model=("profit_model","sum"), pnl_against=("profit_against","sum"), pnl_home=("profit_home","sum"))
            .reset_index().rename(columns={"date":"ds"}).sort_values("ds")
        )
        ...
        with c2[1]: kpi("Best day (model)", f"€{best_day['pnl_model']:+.2f}",
                         delta=str(best_day['ds']), delta_dir="pos")
        with c2[2]: kpi("Worst day (model)", f"€{worst_day['pnl_model']:+.2f}",
                         delta=str(worst_day['ds']), delta_dir="neg")

        # Auto-insight
        best_strategy = max([("Model", roi_model), ("Against-model", roi_against), ("Always-home", roi_home)], key=lambda x: x[1])
        if best_strategy[1] > 0:
            ...
            insight(f"All three strategies are currently unprofitable over this filter window. "
        ...
        fig_bar = go.Figure()
        for col, name, color in [("pnl_model","Model","#3b82f6"),("pnl_against","Against model","#f472b6"),("pnl_home","Always home","#34d399")]:
        ...
        st.dataframe(pnl.sort_values("Date", ascending=False), use_container_width=True, hide_index=True)
